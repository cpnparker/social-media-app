...
/** One slide's preview, walked out of requests that have ALREADY been built.
 *
 *  Split out of toPreviewModel so that a caller holding the requests can read
 *  them back without building the slide a second time. Partly cost — that is
 *  what lets lib/slides/validate.ts measure a deck for one build rather than
 *  two — and mostly correctness: nobody has established that buildSlideRequests
 *  is deterministic, and a guard measuring a different build from the one the
 *  user is shown would be measuring the wrong deck.
 *
 *  `slide` is still required: the caps pass at the bottom decides whether a box
 *  is upper-cased by comparing the drawn text to the spec value it came from,
 *  which is a question the requests alone cannot answer. */
export function previewSlideFrom(slide: SlideInput, requests: any[]): PreviewSlide {
...
export function toPreviewModel(slides: SlideInput[]): PreviewDeck {
  ...
  slides.forEach((slide, i) => {
    out.push(previewSlideFrom(slide, buildSlideRequests(slide, i, `p${i}`)));
  });
