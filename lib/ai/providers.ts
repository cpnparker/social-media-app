...
import { applyEditSlide, unrenderableSlides, undrawnTableSlides, undrawnTableFaults, normaliseSlideForWrite, retireDeclaredBriefs, textReadySlides, PAYLOAD_FIELDS, SlideCallRefusal, blankSlideFaults, payloadsTakenBy, type RefusalScope } from "@/lib/slides/edit";
...
const SLIDE_ITEM_PROPS: Record<string, any> = {
  ...
  comparison: {
    type: "object",
    description: "A comparison table (comparison layout): `columns` are the options across the top (2-4), `rows` are the criteria. A cell of 'yes'/'no' draws a tick or cross; in a grid of only yes/partial/no/n/a cells, 'partial' draws ½ and the slide draws their key itself: never write one. Any other text prints as-is. Highlight the row that makes your case.",
    properties: {
      columns: { type: "array", items: { type: "string" } },
    ...
    parameters: {
      ...
      properties: {
        ...
        fidelity: {
          ...
          description:
            "Set to \"preserve\" whenever the user is CONVERTING something they already have — 'put this deck in our format', 'keep the content the same', 'same slides, just our style', 'just change the format'. Under preserve: ONE source slide becomes ONE output slide, in the source's order, keeping its own wording, figures and slide count. Choose the layout that best CARRIES each slide's content, but do not change what the content IS: do not merge slides, do not invent section dividers or closing slides the source does not have, do not reword titles, do not drop the source's own labels. EVERYTHING VISIBLE ON A SOURCE SLIDE STAYS VISIBLE ON ITS OUTPUT SLIDE — do not demote on-slide text to speaker notes and do not relocate a slide's summary bar or side panels to a different slide; a table slide takes its commentary in `bodyRight`. It also turns off the advice to make the deck more visual, which is written for a deck you are AUTHORING and otherwise pushes you to restructure a conversion. Default is \"restyle\", which is the house REBUILD method: read the whole source first and work out what it is arguing; REWRITE every headline so it states the finding rather than labels the topic ('Three of five pillars score below 50', never 'Results overview'); choose each layout from the content, not from the source's design (a chart that is really four numbers becomes a stat slide, a five-box process diagram becomes cards); one source page is NOT one output slide — dense pages split, thin pages merge; open with the argument in one slide so a reader who stops there still gets it; and every figure must exist in the source — carry ambiguity over and flag it, never resolve it silently.",
        },
        publish: {
          type: "boolean",
          description:
...
/**
 * How many slides the source document has, when one is attached.
 *
 * The pptx reader emits "--- Slide N ---" per slide, so on a CONVERSION the
 * server knows the number the deck is supposed to come out at. That is the
 * only fact in this loop that can hold a preserve conversion to one-for-one:
 * telling the model not to merge slides is advice, and it merged six Quick Win
 * slides into one cards slide anyway. A count it is shown, against a count it
 * produced, is checkable.
 */
export function sourceSlideCount(messages?: AIMessage[]): number {
  if (!messages?.length) return 0;
  for (let i = messages.length - 1; i >= 0; i--) {
    for (const att of messages[i].attachments || []) {
      const text = (att as any).extractedText;
...
async function buildSlidesDraft(
  ...
  onImageProgress?: (done: number, total: number) => void
): Promise<{ draft: { title: string; slides: any[]; preview: PreviewDeck }; geometry: DeckGeometry }> {
  ...
  // MEASURED BEFORE THE PREVIEW IS BUILT, on the deck as it will be drawn:
  // every element on the canvas, no text box on another, no text running out of
  // its box onto the one below. Until now those three assertions existed only
  // in scripts/verify-slide-layouts.ts, over about forty hand-written fixtures,
  // so every LAYOUT was measured and no DECK ever was.
  //
  // Advisory today — geometryRefusal returns null while GEOMETRY_SEVERITY is
  // "advisory", and the seam and what its decision needs are documented there.
  // The call is here rather than behind the constant so that the day it flips,
  // the refusal already happens before a single slide has been shown.
  //
  // AND THE WHOLE BLOCK IS GUARDED, which is not belt and braces: a measuring
  // pass that throws would take the deck with it, and "nothing this validator
  // finds may block a build" has to include the validator itself falling over.
  // The refusal is raised OUTSIDE the guard so that the day the seam flips, a
  // deliberate refusal is not swallowed as a validator fault.
  let geometry: DeckGeometry = {
    faults: [], slidesChecked: 0, elementsChecked: 0, textBoxesChecked: 0,
    inkBoxesChecked: 0, unbuildable: slides.length, ms: 0,
  };
  let refusal: string | null = null;
  try {
    geometry = validateDeck(slides, "draft");
    // Tagged, not titled: one line per built deck is the RATE, and a client
    // deck's title in a log line is not needed to count faults.
    logDeckGeometry(geometry, "draft");
    refusal = geometryRefusal(geometry);
  } catch (e) {
    // Said out loud, because a guard that measures nothing and reports nothing
    // is the failure this repo has already paid for twice.
    console.log(`[SlideGeometry] draft: not measured — ${e instanceof Error ? e.message : String(e)}`);
  }
  if (refusal) throw new SlideCallRefusal(refusal, { scope: "build" });
  return {
    draft: {
      title,
      slides,
...
): Promise<{ title: string; slides: any[]; presentationId?: string; edited: boolean; publishedBefore?: boolean; density: Density; notes?: string[] }> {
...
async function streamAnthropic(
  ...
  //
  // WATCH THE CEILING: SLIDES_GEN_TOOL alone serialises to 54,447 characters
  // in the OpenAI shape and 54,418 in the Anthropic one pushed below, against
  // the TOOL_CEILING of 55,000 asserted in scripts/verify-slide-layouts.ts
  // (checks 20d and 41j) — 553 of headroom. The last 105 were spent on
  // 2026-09-23 telling the model that a grid of marks draws ½ and its own
  // key; the 211 before them, on 2026-09-21, on what the two densities are
  // for.
  ...
  // payloads the schema already declares — `image`/`body`/`bodyRight` plus one
  // string, and `stages`, which `process` already asks for; a layout with a
  // payload of its own costs the ~3,600 a hub-sized one measures and breaches
  // this. Check 41j prints the room left rather than only the size, because
  // this line is the only place the number is written down outside it and it
  // went stale by nine hundred characters inside one stage.
      ...
      if (tool.name === "generate_image") {
        ...
        try {
          ...
          if (!tool.input.publish && !deckPresId) {
            const built = await buildSlidesDraft(deckTitle, deckSlides, messages, onDeckImage);
...
/**
 * Billed output tokens for an xAI chat-completions usage object.
 *
 * xAI REPORTS REASONING TOKENS ON TOP OF `completion_tokens`, NOT INSIDE IT.
 * Its API reference calls `completion_tokens_details.reasoning_tokens`
 * "additional" (docs.x.ai chat-completions reference, checked 2026-09-23).
 * OpenAI's convention is the opposite — "reasoning tokens ... are billed as
 * output tokens" and appear as a BREAKDOWN inside the output count
 * (developers.openai.com, same day). The two APIs are wire-compatible and
 * disagree about this one field.
 *
 * So this belongs to the xAI chain and nowhere else. Adding it in
 * streamOpenAI or streamGemini would DOUBLE-COUNT: Gemini is reached through
 * Google's `/v1beta/openai/` compatibility layer and follows OpenAI's
 * convention. Six call sites share the shape `totalOutputTokens +=
 * u.completion_tokens`; only the two in streamXAIChatCompletions are wrong.
 *
 * What it was costing: grok reasoning defaults to `high` and every measured
 * reasoning row under-reported billed output by 1.8x (low) to 2.6x (high).
 * `calculateCostTenths` then priced the understated figure into a HARD SPEND
 * CAP, so the cap was loose by the same factor on exactly the traffic that
 * costs the most.
 */
