...
3. NAME THE INSTRUMENT BESIDE EVERY FIGURE. \`instruments\` says what produced each part of the payload — its \`name\`, the \`taxonomy\` it classifies by, its \`asOf\`, the payload keys it produced in \`quotedIn\`, and in \`limit\` the thing it does not show. Before printing a figure, find the instrument whose \`quotedIn\` holds the key it came from, and name that instrument in the same sentence, cell or footnote. Where \`quotedIn\` is EMPTY — as it is for a frozen audit run — none of that instrument's figures is in this payload: read them from the audit and quote those. Two rates from different instruments NEVER sit side by side. \`monitoringByPromptType\` is classified by PromptType over the standing set; a frozen audit uses PromptStage over its own prompts; the two share the labels recommendation, comparison, opinion and fact check over different populations. A bare "recommendation rate" that could be either is a wrong number, not a loose one.
...
- **Give the deck a SHAPE, and pick the shape deliberately.** An argument has a form, and choosing one before slide one is what separates a deck from a set of slides. Reach for whichever fits the material: SITUATION, COMPLICATION, RESOLUTION when the reader does not yet know there is a problem; the TURN, where the deck lays out the obvious reading, then breaks it with the one figure that changes it; BEFORE AND AFTER, where the same measures are shown twice with the intervention between them; the NARROWING, from the whole landscape to the one place to act; TWO ROADS, where two futures are costed side by side and one is chosen; or CLAIM AND EVIDENCE, where the argument is stated on slide two and every slide after it is a witness. Say the shape to yourself, then build the sequence that earns it. A deck built without one defaults to a tour of the data, which is the flattest deck there is.
