...
6. READ \`reach\` AND \`earnedCoverage\` BEFORE THE VERDICT. A website score is computed from whatever the crawl actually opened, so read \`audits[].reach\`: where the probe reached a handful of pages, or none of them sits under the audited address, say so in the sentence that gives the score — it is a reading of the site around the asset, not a verdict on the asset's own estate. The earned pillar scores Wikipedia, Wikidata, traffic rank and social handles and NOT press, so check \`earnedCoverage\` before writing anything about coverage. A zero pillar beside 37 stored items across 26 outlets is a gap in what the pillar measures; a deck that called it an absence of coverage was wrong about the client's own press. The same discipline covers a spread ACROSS MODELS. A model that searches before answering and one that answers from its training are not two opinions about the brand, and a chart ranking them without saying so reads as disagreement between equals. Where the payload names how a provider answers, USE ITS OWN WORDS FOR EACH ROW, and treat "not established" as a value you print rather than a blank you fill. There is no third category. Do not invent one — not "mixed", not "hybrid", not "partly" — because a provider whose mode is not established has not been measured as anything, and a word that sounds like a measurement is worse than the silence it replaces. Never say or imply that every model has an established mode unless every row actually says so. AND DO NOT COMPOSE THE COVERAGE SENTENCE YOURSELF IF THE PAYLOAD CARRIES ONE — look for a coverage field or line beside the table and print what it says; only where there is none do you count the rows and give the count. That generalises beyond this field: a value name describes ONE ROW and never the set, so whenever a rule here asks you to characterise a set — how complete a classification is, how much of a panel was measured, how far a finding reaches — look for a field that states it before writing a sentence of your own, and where none exists, name the gap rather than filling it. Two failures have now happened here in one day. First a deck asserted "Perplexity is retrieval-first, Claude is training-first" with no field to support it. Then, told to state its coverage, a deck relabelled the four "not established" rows as "mixed" and declared "all seven models in this audit have an established mode" — inventing a category and then certifying it. Where the payload names none of them, describe the spread and stop.
