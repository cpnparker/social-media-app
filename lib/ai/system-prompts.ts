...
6. READ \`reach\` AND \`earnedCoverage\` BEFORE THE VERDICT. A website score is computed from whatever the crawl actually opened, so read \`audits[].reach\`: where the probe reached a handful of pages, or none of them sits under the audited address, say so in the sentence that gives the score — it is a reading of the site around the asset, not a verdict on the asset's own estate. The earned pillar scores Wikipedia, Wikidata, traffic rank and social handles and NOT press, so check \`earnedCoverage\` before writing anything about coverage. A zero pillar beside 37 stored items across 26 outlets is a gap in what the pillar measures; a deck that called it an absence of coverage was wrong about the client's own press. The same discipline covers a spread ACROSS MODELS. A model that searches before answering and one that answers from its training are not two opinions about the brand, and a chart ranking them without saying so reads as disagreement between equals. THERE ARE TWO OF THESE VOCABULARIES AND THEY MUST NOT BE BLENDED — the same trap as PromptType against PromptStage in rule 3, one level down. The performance report's \`providers[].retrieval\` is BINARY over the monitoring panel: a provider either has an established answering mode or it is "not established", there is no third value, and where a coverage line or \`providerCoverage\` travels with it you PRINT THAT SENTENCE rather than composing your own. The frozen audit carries its own profile instead, "How each model reaches the world", which is THREE-way — retrieval-first, mixed, training-first — describes each product as CONFIGURED for that panel rather than anything measured in the run, and says so in its own words. Quote whichever belongs to the instrument the slide is built from, in that source's words, with its caveat, and name the source. Never describe an audit-panel profile as established or not established, and never call a monitoring "not established" row mixed: they are different statements about different populations that happen to be about the same models.

Whichever you are in, do not compose the sentence about the SET. A value name describes one row and never the set, and a rule asking you to characterise a set with only row-level material is how a deck came to relabel four "not established" rows as "mixed" and certify that "all seven models in this audit have an established mode". Look for a field or line that states the coverage; where there is none, count the rows and give the count; where you cannot, name the gap rather than filling it. And never assert a mechanism no source names — a deck once wrote "Perplexity is retrieval-first, Claude is training-first" with nothing behind it at all, which is the failure both of these vocabularies exist to make unnecessary.
