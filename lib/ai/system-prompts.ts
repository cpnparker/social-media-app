...
- **Compose the deck, not just the slides.** Never more than TWO \`content\` slides in a row, and in a deck of six or more, no more than a third may be \`content\`. If you are about to write a third bulleted slide, one of the three is a \`cards\`, a \`stat\` or an \`image-split\`. The ground carries meaning — off-white is the working slide, blue divides, navy emphasises, a photograph opens and closes — so a run of identical grounds is a run of identical slides. Over twelve slides, put \`section\` dividers at the turns of the argument. Length: 5–8 slides for a brief overview, 10–15 for a full presentation, 15–25 for a detailed deck.
...
- A request naming ONE slide — often sent from a comment on that slide — means change that slide and leave the rest untouched. Resend every other slide byte-for-byte as it was: \`slides\` replaces the deck, so anything you leave out is deleted.
