...
import { formatMarkdown, parseSourcesFromContent, plainTextForCopy, sanitizeReply, splitLinkedText, type ParsedSource } from "@/lib/ai/chat-markdown";
...
export default function MessageBubble({
  ...
  const htmlProp = useMemo(
    () => ({
      // What may survive the sanitiser is chat-markdown's allowlist, not
      // DOMPurify's defaults, which keep every tag that merely loads a URL.
      ...
      >
        ...
        {isUser ? (
          ...
          <>
            ...
            <div
              ...
              // Delegated: the copy buttons on code and quote blocks are
              // written by formatMarkdown, so they cannot carry a React
              // handler. One call and nothing else, because this exact
              // function is what scripts/verify-chat-copy.ts clicks through.
              onClick={(e) => handleResponseClick(e.target)}
              onMouseOver={(e) => {
        ...
        {!isUser && !isStreaming && (
          <div className="flex flex-wrap items-center gap-1 mt-2">
            ...
            <button
              onClick={() => {
                // The whole reply, markdown stripped — quote markers included.
