...
**This is an expansion of access, and that should be deliberate.** Measured after deployment: **142 meetings are readable company-wide today; 435 after the change — a 3.1× expansion.** The gain is that team meetings stop being invisible to the team. The cost is that ~293 internal meetings become readable by everyone, and the personnel screen is the only thing standing between that and a leadership conversation.

**Two numbers here are easy to confuse, and I confused them.** The rule classifies **1,266 events** as `team`. Only **532** are *shareable*, because `get_visible_meetings` also requires a summary and JSON-parseable attendees — those gates drop 734, nearly all unprocessed meetings. An earlier version of this document predicted "~1,150, an 8× expansion" by quoting the classification count as though it were the sharing count. The shared set will drift upward toward 1,266 as MeetingBrain processes its backlog, so this is not a fixed number.

The personnel screen trips on **6.7%** of meetings overall, but on the internal group meetings it actually gates it withholds **97 of 390 — 25%**. That is much higher than the corpus rate and worth watching: it is the safe direction, and there is a live restructure generating genuine hits, but a quarter of team meetings staying invisible is a real cost. Notably it is **not** the titles doing it (title-only catches ~0%) but the summaries, which is where the substance is. Two terms were removed after measuring which ones did the work: `pip` matched "pip install" across 94 rows, and bare `promotion` matched 78, which in a content marketing agency is overwhelmingly a campaign.
