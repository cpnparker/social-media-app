...
**Owner amendments (2026-08-25, after review):**
1. **The third type is never named in the product.** Internally it stays
   `type_content='cv'`; no UI string, chip, tooltip, or empty state ever renders
   the word. It is not offered on the start screen, and detection applies it
   SILENTLY — the studio simply behaves correctly (no score, a Checks list)
   without saying why. This is the one deliberate exception to the
   detection-proposes-a-person-confirms rule, and `verify-optimizer-types`
   asserts the absence: no rendered string in the studio's components may
   contain the type's name.
2. **Client style is first-class.** Selecting a client imports their STYLE as
   well as their canon — see "Client style" under §3. The
   `optimizer_client_canon.document_voice` column has existed since the schema
   shipped and is written by nothing; it becomes the style card's home.
...
Registry `lib/optimizer/content-types.ts` as designed (pillars re-weighted to 10000, criteria filtered from the shared universe before the engine runs, one byte-stable `judgeSystem` per type, versions stamped `"<type>@<version>"`), **plus a chrome contract** derived from `analyses` **(Δ V2.2)**: `judge:false` hides the assessment chip (page.tsx:1012-1045), the Assess button (:1081-1086), and the IssueList assess CTA (:102-117); `coverage:false` hides the Coverage tab (:1176-1182); the quiet type replaces the graded Score circle with the Checks list, renders no type chip at all, and its detection is applied silently rather than proposed (owner amendment 1). `type_content` joins `assessmentKeyWith`'s parts (judge.ts:589-607, which already carries `input.format` and `JUDGE_MODEL`) and `coverageKey` (coverage.ts:367), pipeline versions bumped. An analysis a type turns off 400s **before** `assertServiceAllowed`. Detection: deterministic heuristics from source priors + parsed structure; disagreement resolved by one memoised closed-label call on **gpt-5.6-luna** (never grok-4-1-fast — retired slug redirected and billed at grok-4.3's $1.25/$2.50, model-costs.ts). **Δ V3.6:** the assess route's 413 copy ("calibrated for 800-2,500 words", assess/route.ts:108-115) becomes type-aware — a report writer is not told their document should be 2,500 words; report behaviour at the 40k cap is Open Decision 1.

### Client style (owner amendment 2 — key functionality, Stage 1)

Selecting a client already loads their CANON — facts, brand names and aliases,
suggested queries (`buildClientCanon`, lib/optimizer/client-canon.ts:69;
snapshot into `config_canon` at session create so assessments stay
reproducible). What it does NOT load is how the client SOUNDS:
`optimizer_client_canon.document_voice` has existed since the schema shipped
(20260821 sql:230) with a default of `''` and **no writer and no reader** — a
column-shaped intention. It becomes real:

- **Derivation.** On client selection (an explicit act, so billed to the
  existing `optimizer` service row — Stage 1 adds no automatic spend), if
  `document_voice` is empty or `date_refreshed` is stale (>30 days), one
  claude-sonnet-5 call derives a STYLE CARD from the client's most recent
  finalised drafts in this studio plus canon facts: tone and register, sentence
  rhythm, vocabulary the client uses and vocabulary they avoid, first- vs
  third-person conventions, capitalisation of product names. ~600 tokens,
  written to `document_voice`, memoised by the same refresh discipline as the
  canon. Where the client has no prior content the card says so — a thin style
  must look thin, the canon's own `gaps` rule (client-canon.ts:50-52).
- **Consumption.** The card enters the byte-stable cached prefix of every
  generation surface — ghost, finish, generate, suggest — in the
  session+canon+sources block (the Δ V3.3 serialization), so the client's voice
  costs cache-read prices after the first call. The judge's tone criteria read
  it too: "matches the client's register" becomes checkable against a stated
  register rather than a vibe.
- **Visibility.** The Sources rail shows "Client style — {name}" as a card with
  the derived summary and a Refresh action; the Set up step shows the client
  selector first among equals. A session with no client selected writes in the
  workspace's neutral register and says so.
- **Editing.** The card is text; the owner of the session can edit it, and an
  edited card is never overwritten by refresh without confirmation — a
  hand-tuned voice is a deliberate artifact, not a cache entry.
- *Verify (Stage 1):* `verify-optimizer-types` gains: the style card reaches
  the generation prompt's STABLE region (byte-position asserted, not grepped);
  an empty `document_voice` never injects an empty block (the cache prefix must
  not change shape between clients with and without style); the refresh
  overwrite guard holds for an edited card.
...
**Stage 1 — the typed writing studio (no new automatic spend).** Start-screen flip; Set up step with the client selector first among equals; client STYLE derivation on selection + `document_voice` write/read + style card UI (owner amendment 2); `type_content` column + registry + heuristic detection + confirm chip; **per-type chrome gating (Δ V2.2)**; per-type engine/judge gating with 400-before-spend; memo-key extension; type-aware 413 copy (Δ V3.6); finish-section/document with the `generating` claim and honest pricing (Δ V1.5); health pill (dot-only lg+, Δ V2.8); header collapse order specified; `requireWritable` introduced as a no-op.
...
- **No score from the cheap tiers.** Hints never enter `optimizer_assessments`; the Sonnet judge remains the only number on screen — and for the quiet type, no number renders at all (Δ V2.2).
...
- **No silent type switches between the NAMED types.** Detection proposes; a person confirms (imports confirm via the header chip, Δ V2.7). The unnamed type is the deliberate exception: applied silently, never rendered (owner amendment 1).
- **No grammar/plagiarism/SEO ambitions.** Tier 0 stays the GEO-mechanical engine.
- **Coverage and page audit do not extend beyond articles**; the audit route stays model-free but view-blocked.
