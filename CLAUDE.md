...
Eleven scripts guard `lib/optimizer/` and the import/export paths. Run all of them
before shipping anything that touches the rubric, the judge, anchoring or how
content gets in:

```
for f in rubric anchors judge gate doc-index highlight import export import-html live url; do npx tsx scripts/verify-optimizer-$f.ts || break; done
```
